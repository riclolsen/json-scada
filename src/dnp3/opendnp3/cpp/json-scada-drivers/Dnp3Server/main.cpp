...
public:
    ...
    int __cdecl main(int argc, char* argv[])
    {
        ...
        for (auto& dnp3Conn : dnp3Connections)
        {
            ...
            if (dnp3Conn.autoCreateTags)
            {
                ...
                for (auto&& doc : resTagsDig)
                {
                    ...
                    db["realtimeData"].update_one(
                        ...
                        make_document(kvp(
                            "$push",
                            make_document(kvp("protocolDestinations",
                                              make_document(kvp("protocolDestinationConnectionNumber",
            ...
            DatabaseConfig cfg
                = database_by_sizes(lastBinaryInput + 1, lastDoubleBinaryInput + 1, lastAnalogInput + 1,
                                    lastCounter + 1, lastFrozenCounter + 1, lastBinaryOuputStatus + 1,
                                    lastAnalogOutputStatus + 1, lastTimeAndInterval + 1, lastOctetString + 1);
