<template>
        ...
        <v-data-table-server :headers="headers" :items="tags" :items-per-page="itemsPerPage" :page="page"
            ...
            <template v-slot:top>
                <v-toolbar flat class="d-print-none">
                    ...
                    <v-btn color="blue" size="small" variant="flat" @click="newTag()">
    ...
    <v-dialog scrollable v-model="dialogDelete" max-width="500px">
        <v-card>
            ...
            <v-card-actions>
                <v-spacer></v-spacer>
                <v-btn color="orange darken-1" text variant="tonal" @click="closeDelete">{{
    ...
    <v-dialog scrollable v-model="dialog" max-width="750px">
        <v-card>
            <v-card-title>
            ...
            <v-card-text>
                <v-text-field v-model="editedItem._id" :label="$t('admin.tags.editId')"></v-text-field>
                <v-text-field v-model="editedItem.tag" :label="$t('admin.tags.editName')"></v-text-field>
                <v-text-field v-model="editedItem.description" :label="$t('admin.tags.editDescription')"></v-text-field>
                <v-text-field v-model="editedItem.group1" :label="$t('admin.tags.editGroup1')"></v-text-field>
                <v-text-field v-model="editedItem.group2" :label="$t('admin.tags.editGroup2')"></v-text-field>
                <v-text-field v-model="editedItem.group3" :label="$t('admin.tags.editGroup3')"></v-text-field>
                <v-select :items="['supervised', 'command', 'calculated', 'manual']"
                    :label="$t('admin.tags.editOrigin')" v-model="editedItem.origin" class="ma-0"></v-select>
                <v-select :items="['digital', 'analog', 'string', 'json']" :label="$t('admin.tags.editType')"
                    v-model="editedItem.type" class="ma-0"></v-select>
                <v-text-field v-if="editedItem.type == 'digital'" v-model="editedItem.stateTextFalse"
                    :label="$t('admin.tags.editStateTextFalse')"></v-text-field>
                <v-text-field v-if="editedItem.type == 'digital'" v-model="editedItem.stateTextTrue"
                    :label="$t('admin.tags.editStateTextTrue')"></v-text-field>
                <v-text-field v-if="editedItem.type == 'digital'" v-model="editedItem.eventTextFalse"
                    :label="$t('admin.tags.editEventTextFalse')"></v-text-field>
                <v-text-field v-if="editedItem.type == 'digital'" v-model="editedItem.eventTextTrue"
                    :label="$t('admin.tags.editEventTextTrue')"></v-text-field>
                <v-text-field v-if="editedItem.type == 'analog'" v-model="editedItem.unit"
                    :label="$t('admin.tags.editUnit')"></v-text-field>

                <v-text-field type="number" v-if="['supervised'].includes(editedItem.origin)"
                    v-model="editedItem.commandOfSupervised"
                    :label="$t('admin.tags.editCommandOfSupervised')"></v-text-field>
                <v-text-field type="number" v-if="['command'].includes(editedItem.origin)"
                ...
                <v-select :items="protocolConnections" :label="$t('admin.tags.editProtocolSourceConnectionNumber')"
                    ...
                    v-if="['supervised', 'command'].includes(editedItem.origin)"
                    v-model="editedItem.protocolSourceConnectionNumber" class="ma-0"></v-select>

                <v-text-field v-if="['supervised', 'command'].includes(editedItem.origin)"
                ...
                <v-text-field v-if="['command'].includes(editedItem.origin)"
                    v-model="editedItem.protocolSourceCommandDuration"
                    :label="$t('admin.tags.editProtocolSourceCommandDuration')"></v-text-field>

                <v-text-field type="number" v-if="['supervised', 'command'].includes(editedItem.origin)"
                    v-model="editedItem.kconv1" :label="$t('admin.tags.editKconv1')"></v-text-field>
                <v-text-field type="number" v-if="['supervised', 'command'].includes(editedItem.origin)"
                    v-model="editedItem.kconv2" :label="$t('admin.tags.editKconv2')"></v-text-field>
                ...
                " v-model="editedItem.protocolSourceCommandUseSBO" inset color="primary" :label="`${$t('admin.tags.editProtocolSourceCommandUseSBO')}${editedItem.protocolSourceCommandUseSBO
                    ? $t('admin.tags.editProtocolSourceCommandUseSBOTrue')
                    : $t('admin.tags.editProtocolSourceCommandUseSBOFalse')
                    }`" class="mt-0"></v-switch>

                <v-switch v-if="['supervised'].includes(editedItem.origin)" v-model="editedItem.isEvent" inset
                    color="primary" :label="`${$t('admin.tags.editIsEvent')}${editedItem.isEvent
            ...
            <v-card-actions>
                <v-spacer></v-spacer>
                <v-btn color="orange darken-1" text variant="tonal" @click="close">
                    {{ $t('common.cancel') }}
                </v-btn>
                <v-btn color="blue darken-1" text variant="tonal" @click="save">
                    {{ $t('common.save') }}
                </v-btn>
            </v-card-actions>
        </v-card>
