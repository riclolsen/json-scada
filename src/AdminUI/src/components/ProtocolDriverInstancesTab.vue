<template>
  <v-container fluid class="protocol-driver-instances-tab">
    ...
    <v-data-table :headers="headers" :items="driverInstances" :items-per-page="5" class="mt-4 elevation-1"
      ...
      <template #[`item.actions`]="{ item }">
        ...
        <v-icon size="small" @click="openDeleteProtocolDriverInstanceDialog(item)">
          mdi-delete
        </v-icon>
      </template>
      ...
      <template #[`item.process`]="{ item }">
        <v-icon v-if="!item.running" disabled size="small" class="me-2" @click="startProtocolDriverInstance(item)">
          mdi-play
        </v-icon>
        <v-icon v-if="item.running" disabled size="small" class="me-2" @click="stopProtocolDriverInstance(item)">
          mdi-stop
        </v-icon>
      </template>
    </v-data-table>
  ...
  <v-dialog v-model="dialogDeleteInstance" max-width="400px">
    <v-card>
      <v-card-title>{{ $t('admin.protocolDriverInstances.deleteInstance') }}</v-card-title>
      <v-card-text>
        {{ $t('admin.protocolDriverInstances.deleteInstanceConfirm') }}
      </v-card-text>
      <v-card-text>
        {{ editedInstance.protocolDriver + " / " + editedInstance.protocolDriverInstanceNumber }}
      </v-card-text>
      <v-card-actions>
        <v-spacer></v-spacer>
...
async function fetchProtocolDriverInstances() {
  ...
  try {
    ...
    json.forEach((item, index) => {
      ...
      item.localTimeUpdate = item.activeNodeName + " " + (item.activeNodeKeepAliveTimeTag ? new Date(item.activeNodeKeepAliveTimeTag).toLocaleString() : '');
      item.running = new Date(item.activeNodeKeepAliveTimeTag).getTime() > new Date().getTime() - 10000;
    });
...
async function updateProtocolDriverInstance() {
  ...
  const dup = Object.assign({}, editedInstance.value);
  if ('id' in dup) { delete dup.id; }
  if ('nodesText' in dup) { delete dup.nodesText; }
  if ('running' in dup) { delete dup.running; }
  if ('localTimeUpdate' in dup) { delete dup.localTimeUpdate; }
  if ('process' in dup) { delete dup.process; }
  if ('stats' in dup) { delete dup.stats; }
  if ('activeNodeName' in dup) { delete dup.activeNodeName; }
  if ('activeNodeKeepAliveTimeTag' in dup) { delete dup.activeNodeKeepAliveTimeTag; }
  if ('softwareVersion' in dup) { delete dup.softwareVersion; }
  try {
