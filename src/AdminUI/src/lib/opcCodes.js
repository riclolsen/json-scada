/*
 * OPC-UA Standard Codes (and some extensions) — ES module port of public/opc-codes.js
 * {json:scada} - Copyright 2020-2026 - Ricardo L. Olsen
 *
 * Used by the native Tabular/Alarms viewer to speak the OPC Web-HMI protocol
 * over POST /Invoke/ (the same envelope the legacy tabular.html uses).
 */
...
// Pseudo-point keys used by the alarms viewer for beep/update status
export const BEEP_POINTKEY = -1
export const CNTUPDATES_POINTKEY = -2

// Point quality flag bitmask (built by the viewer from the realtimeData properties)
export const Flags = {
  ANALOG: 0x20,
  FAILED: 0x80,
  ALARMED: 0x100,
  ANNOTATION: 0x200,
  ALARM_DISABLED: 0x400,
  ABNORMAL: 0x800,
  MANUAL: 0x0c,
  FROZEN: 0x1000,
  INTERLOCKED: 0x2000,
