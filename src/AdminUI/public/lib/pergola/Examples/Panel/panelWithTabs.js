...
myPanel.populate = {
	...
	"tab # 2" : function () {
	  $C({element : "text", x : this.owner.width / 2, y : 200, 'font-size' : 120, "font-weight" : "bold", fill : "#F0F0F0", 'text-anchor' : "middle", textNode : this.title.toUpperCase(), appendTo : this.pane.container});
	},
	"HTML table" : function () {
		var switchTag = $C({element : "switch", appendTo : this.pane.container}),
	  		fObj = $C({element : "foreignObject", width : this.pane.width, height : this.pane.height, appendTo : switchTag}),
	  		body = pergola.HTML_FO({element : "body", xmlns : pergola.ns.xhtml, appendTo : fObj}),
	  		table = pergola.HTML_FO({element : "table", border : 1, cellpadding : 2, cellspacing : 2, style : "width: 100%; background-color: #F8F8F8;", appendTo : body});

	  for (var i = 0; i < 3; i++) {
	    var tr = pergola.HTML_FO({element : "tr", appendTo : table});
	    for (var j = 0; j < 3; j++) {
	      var td = pergola.HTML_FO({element : "td", style : "height: " + parseInt((this.pane.height - 40) / 3) + "px; text-align: center;", appendTo : tr});
	      var p = pergola.HTML_FO({element : "p", style : "font-size: 16pt; font-weight: bold; color: #F0F0F0;", textNode : ("CELL " + (i * 3 + j)), appendTo : td});
	    }
	  }

	  $C({element : "text", x : this.pane.width / 2, y : 80, 'font-size' : 16, 'text-anchor' : "middle", textNode : "<foreignObject> is not implemented in Internet Explorer Trident", appendTo : switchTag});
	}
};
