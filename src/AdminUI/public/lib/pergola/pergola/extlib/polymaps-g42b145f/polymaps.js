...
//==========================================  WHEEL  ==============================================
	wheel : function() {
		...
		function mousewheel(e) {
			var delta = (e.wheelDelta / 120 || -e.detail) * .05,
					point;
...
/* If smooth zooming is disabled, batch events into unit steps. */
			...
			if (delta) {
				...
				zoomValue = map.zoom();
        if (statusBar) statusBar.zoom(zoomValue);
        if (zoomValue.isInteger()) win.mapZoomLevelMark(zoomValue);
			}
			e.preventDefault();
...
//==========================================  GRID  ==============================================
	grid : function() {
		var grid = {},
				...
				size = win.map.size(),
				ctm = win.doc.ctm,
				g = $C({element: "g", fill: "#D0D000", "font-style": "italic", "font-family": "'Times New Roman'", "font-size": "10px", display: "none"}),
        ...
        lonText, latText;
				...
				$C({element: "rect", fill: "url(#LonGutter)", opacity: .75, width: size.x, height: 20, appendTo: lonG});
				$C({element: "rect", fill: "url(#LatGutter)", opacity: .75, width: 22, height: size.y, appendTo: latG});
      ...
      while (ticksG.firstChild) ticksG.removeChild(ticksG.firstChild);
			while (lonText.firstChild) lonText.removeChild(lonText.firstChild);
			while (latText.firstChild) latText.removeChild(latText.firstChild);
			...
			for (var y; (y = map.locationPoint(nw).y) <= size.y; nw.lat--) {
				...
				if (draw) {
					...
					$C({element: "text", x: 3, y: y + 3, textNode: nw.lat, appendTo: latText});
				}
			}
		}
